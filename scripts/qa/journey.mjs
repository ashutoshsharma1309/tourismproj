...
const exploreCta = page.getByRole("link", { name: /^Explore destinations$/ }).first();
...
check("TEST 03: all 15 destinations offer selection", addButtons === 15, `${addButtons} controls`);
...
await addButton("Kyoto").click();
await page.waitForTimeout(250);
check("TEST 05: Mumbai and Kyoto are both selected",
  await inJourney("Mumbai").count() > 0 && await inJourney("Kyoto").count() > 0);
...
check("TEST 06: removing Mumbai leaves only Kyoto",
  /1 destination selected/i.test(afterRemoval) && /Kyoto/.test(afterRemoval) && !/Mumbai/.test(afterRemoval),
  afterRemoval.slice(0, 70));

await addButton("Paris").click();
await addButton("Rome").click();
...
check("TEST 07: chips keep the order they were chosen in",
  chips.length === 3 && /Kyoto/.test(chips[0]) && /Paris/.test(chips[1]) && /Rome/.test(chips[2]),
...
await page.goto(`${BASE}/destinations`, { waitUntil: "load" });
...
check("It opens the FIRST chosen destination, not a combined page",
  new URL(page.url()).pathname === "/destinations/kyoto",
...
for (const [label, raw] of [
  ...
  ["an oversized list", JSON.stringify({ destinations: Array(500).fill("paris"), interests: [] })],
]) {
...
await page.goto(`${BASE}/destinations/mumbai`, { waitUntil: "load" });
await page.waitForTimeout(1100);
...
for (const d of ["kolkata", "delhi"]) {
  await page.goto(`${BASE}/destinations/${d}`, { waitUntil: "load" });
  await page.waitForTimeout(900);
...
check("Confirming the reset clears it",
  /No journey yet/i.test(await page.locator("main").innerText()));

/* A single-destination journey gets no multi-destination furniture. */
await page.goto(`${BASE}/`, { waitUntil: "load" });
await page.waitForTimeout(900);
await page.getByRole("button", { name: /Add Kyoto to your journey/i }).click();
await page.waitForTimeout(250);
await page.goto(`${BASE}/destinations/kyoto`, { waitUntil: "load" });
await page.waitForTimeout(1000);
