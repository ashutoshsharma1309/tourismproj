...
console.log("DESKTOP 1440×900");
{
  ...
  await check("/stories loads on a cold context", async () => {
    const response = await page.goto(`${BASE}/stories`, { waitUntil: "networkidle" });
  ...
  await check("story cards render with images", async () => {
    const cards = page.locator('main a[href^="/stories/"]');
  ...
  await check("FIRST-CLICK: 'Read story' opens that story, once", async () => {
    ...
    const card = page.locator('main a[href^="/stories/"]').first();
    ...
    assert(page.url().includes("/stories/"), `landed on ${page.url()}`);
    return `${href} → “${heading}”`;
  });
  ...
  await check("STORY → MAP: a place link deep-links the explore map", async () => {
    ...
    for (const slug of [
      ...
      await page.goto(`${BASE}/stories/${slug}`, { waitUntil: "domcontentloaded" });
      const link = page.locator('a[href^="/explore?place="]').first();
  ...
  await check("back button returns to the story", async () => {
    ...
    assert(page.url().includes("/stories/"), `back landed on ${page.url()}`);
  ...
  await check("MAP → STORY: a marker's story link opens the story", async () => {
    await page.goto(`${BASE}/explore?place=yuksom`, { waitUntil: "networkidle" });
    await page.waitForTimeout(1200);
    const storyLink = page.locator('main a[href^="/stories/"]').first();
...
console.log("\nRESPONSIVE SWEEP");
{
  for (const width of [375, 390, 430, 768, 1024, 1280, 1440]) {
    ...
    const page = await context.newPage();
    for (const path of ["/stories", "/stories/the-throne-of-stone-at-norbugang", "/explore"]) {
      await page.goto(`${BASE}${path}`, { waitUntil: "networkidle" });
      if (path === "/explore") await page.waitForTimeout(1200);
      const overflow = await page.evaluate(
        () => document.documentElement.scrollWidth - window.innerWidth,
      );
