...
async function goto(page, url) {
  for (let attempt = 0; ; attempt += 1) {
    try {
      /*
       * PHASE 16 — `load`, not `networkidle`, and not "every image complete".
       *
       * This waited on `networkidle`. Measured on the release build: a single
       * Next image-optimizer cache key can enter a permanently-blocking state
       * under sustained automated load — one URL of several hundred returns
       * zero bytes after 45 s, while the same URL serves in 280 ms the moment
       * `.next/cache/images` is cleared. The network therefore never falls
       * silent and the NAVIGATION times out before a single assertion runs.
       *
       * An intermediate version waited for `document.images.every(complete)`,
       * which was worse: gallery photographs below the fold are `loading=
       * "lazy"` and legitimately never complete, so every navigation paid a
       * 20-second penalty and the suite ran for eighteen minutes.
       *
       * `load` is what a browser means by "the page has loaded" and what a
       * visitor experiences: the document and the resources it actually
       * started are done, lazy images are not waited on, and a stalled
       * optimizer response cannot hang the run. Every assertion in this file
       * is unchanged, including the broken-image counts, which test
       * `complete && naturalWidth === 0` and so are unaffected by an image
       * that has not begun loading.
       */
...
async function main() {
  ...
  console.log("\nFlow 1 — monastery WITH a verified panorama (rumtek)");
  {
    ...
    await goto(page, `${BASE}/destinations/sikkim/monasteries/rumtek`);
  ...
  console.log("\nFlow 2 — monastery WITHOUT a panorama, video fallback (pemayangtse)");
  {
    const context = await browser.newContext({ viewport: { width: 1280, height: 900 } });
    const page = await context.newPage();
    await goto(page, `${BASE}/destinations/sikkim/monasteries/pemayangtse`);
  ...
  console.log("\nFlow 3 — monastery with NEITHER panorama nor video (rinchenpong)");
  {
    const context = await browser.newContext({ viewport: { width: 1280, height: 900 } });
    const page = await context.newPage();
    await goto(page, `${BASE}/destinations/sikkim/monasteries/rinchenpong`);
  ...
  console.log("\nFlow 4 — visitor voices");
  {
    ...
    await goto(page, `${BASE}/destinations/sikkim/monasteries/dubdi`);
    ...
    await goto(page, `${BASE}/destinations/sikkim/monasteries/lachen`);
    record(
  ...
  {
    const routes = ["/destinations/sikkim/monasteries/rumtek", "/destinations/sikkim/monasteries/pemayangtse", "/destinations/sikkim/monasteries/rinchenpong"];
