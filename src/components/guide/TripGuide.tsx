 ...
 * map exactly onto what `/planner/result` accepts, so the guide hands off to
...
const INTERESTS = ["Monasteries", "Culture", "Lakes", "Food", "Adventure", "Trekking"] as const;
const STYLES = ["Solo", "Couple", "Family", "Group"] as const;
const DURATIONS = [3, 4, 5, 7, 10, 14] as const;

const OPENING: GuideBlock[] = [
  {
    kind: "text",
    text: "Tashi delek. I can find monasteries, places and stories, explain permits and the visitor fee, and plan a route with you.",
  },
  {
    kind: "note",
...
export function TripGuide() {
  ...
  const [turns, setTurns] = useState<Turn[]>([
    { id: 0, role: "guide", blocks: OPENING },
  ]);
  ...
  const startPlan = useCallback(() => {
    ...
    push({
      role: "guide",
      blocks: [{ kind: "text", text: "How many days do you have in Sikkim?" }],
    });
  }, [push]);
  ...
  return (
    <>
      {/* Bottom-RIGHT. The ambience control holds the left corner. */}
      {!open ? (
        <button
          type="button"
          ...
          className="fixed right-4 bottom-4 z-50 flex h-12 items-center gap-2 rounded-full border border-accent/40 bg-surface-inverse/95 px-4 text-small font-medium text-foreground-inverse shadow-overlay backdrop-blur transition-all hover:border-accent hover:text-accent"
        >
