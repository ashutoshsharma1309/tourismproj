...
export function generateStaticParams() {
  return historyEvents.map((event) => ({ slug: event.slug }));
}
...
export default async function HistoryEventPage({ params }: PageProps) {
  ...
  const stories = event.relatedStories.map((s) => getStoryBySlug(s)).filter((s) => s !== undefined);
  const relatedPlaces = event.relatedPlaces
          ...
          {media ? (
            <figure>
              ...
              <figcaption className="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-1 text-caption text-subtle">
                <Link href={`/archive/${media.id}`} className="text-primary hover:underline">
                  {media.title}
                </Link>
