...
export default async function ExplorePage({
  ...
  const placePhotos = showcasePhotos(20, (photo) => photo.scope === "place").map(
    (photo) => ({ ...photo, href: `/destinations/${destinationId}/explore?place=${photo.slug}` }),
  );
    ...
    <>
      <main id="main" className="mx-auto max-w-7xl px-4 pt-28 pb-20 md:px-6">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href={`/destinations/${destinationId}/stories`} className={buttonClasses({ variant: "outline", size: "sm" })}>
            <BookOpen className="size-4" aria-hidden />
            Read the stories
            <ArrowRight className="size-4" aria-hidden />
          </Link>
          <Link href={`/destinations/${destinationId}/monasteries`} className={buttonClasses({ variant: "ghost", size: "sm" })}>
            Monastery catalogue
            <ArrowRight className="size-4" aria-hidden />
          </Link>
        </div>
        ...
        <section aria-labelledby="map-integrity" className="mt-14 rounded-xl border bg-surface p-6">
          ...
          <Link
            href={`/destinations/${destinationId}/preservation`}
