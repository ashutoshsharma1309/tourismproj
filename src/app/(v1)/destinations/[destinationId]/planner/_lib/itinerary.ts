...
function resolveStop(
  ...
  /* The destination that owns this itinerary. Threaded rather than assumed:
     these hrefs used to be hardcoded to Sikkim, which was invisible only
     because the planner generates for Sikkim alone today. */
...
export function buildItinerary(prefs: PlannerPreferences, destinationId: string): Itinerary {
  ...
  const resolvedDays: ResolvedDay[] = plan.dayPlans.map((day) => {
    ...
    let stops = day.stops
      .map((stop) => resolveStop(stop, destinationId))
    ...
    if (day.travel) {
      ...
      travel = {
        ...
        via: day.travel.via
          ...
          .map((place) => ({ name: place.name, href: `/destinations/${destinationId}/places/${place.slug}` })),
        straightLine:
